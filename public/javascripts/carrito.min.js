...
$("#comprobante").change(function () {
    console.log($('#comprobante').prop("files"));
    if ($('#comprobante').prop("files").length === 0) {
        cssInputInvalid('comprobante', 'Campo requerido, suba una imagen del comprobante de pago');
    } else {
        cssInputValid('comprobante');
        $('#error-comprobante').hide();
    }
});
...
function completarOrden(id_pedido) {
    ...
    myModal.show();
    var dato_archivo = $('#comprobante').prop("files")[0];
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            console.log(response);
            if (response.ok) {
                $('#msg-pedido').text(response.msg)
                modalOC = new bootstrap.Modal(document.getElementById('orden-completada'), {
                    keyboard: false
                })
                modalOC.show();                
                //$.NotificationApp.send("Bien hecho!", response.msg, "bottom-right", "rgba(0,0,0,0.2)", "success");
